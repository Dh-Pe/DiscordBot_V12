...
.setColor(config.COLOR);
    ...
    if (serverQueue) {
        if(serverQueue.songs.length > Math.floor(config.QUEUE_LIMIT - 1) && config.QUEUE_LIMIT !== 0) {
      return message.channel.send(`You can not add songs more than ${config.QUEUE_LIMIT} in queue`)
    }
